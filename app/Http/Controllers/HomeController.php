...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $buildings = [];
        $buildingConfigs = collect(config('school.buildings', []));

        foreach ($buildingConfigs as $config) {
            $prefixes = collect($config['match_prefixes'] ?? [])
                ->filter(fn ($prefix) => is_string($prefix) && $prefix !== '')
                ->map(fn ($prefix) => strtoupper($prefix))
                ->values();

            $codePrefix = (string) $prefixes->first();
            if ($codePrefix === '') {
                continue;
            }

            $buildingName = (string) ($config['home_name'] ?? $config['label'] ?? $config['code'] ?? $codePrefix);
            $buildings[] = [
                'name' => $buildingName,
                'rooms' => Classroom::where('code', 'like', "{$codePrefix}%")
    ...
    public function store(StoreBookingRequest $request)
    {
        ...
        try {
            ...
            $errorMessage = (string) ($e->errorInfo[2] ?? '');

            if ($sqlState === '23000' && $driverCode === '1062' && str_contains($errorMessage, 'uk_booking_date_time_slot')) {
