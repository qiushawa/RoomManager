<template>
    <div
        class="flex h-full flex-col overflow-hidden rounded-2xl border border-a-border-card bg-a-surface shadow-sm transition-all duration-200 hover:border-primary/30"
    >
        <!-- Header -->
        <div
            class="flex items-center justify-between border-b border-a-divider bg-a-surface-2 px-5 py-3.5"
        >
            <div class="flex items-center gap-3">
                <span
                    class="flex h-7 w-7 items-center justify-center rounded-lg bg-primary/15 text-[11px] font-bold tracking-wide text-primary"
                >
                    {{ buildingCode }}
                </span>
                ...
                <span
                    class="rounded-full border border-a-divider bg-a-surface px-2 py-0.5 text-[11px] text-a-text-dim tabular-nums"
                >
                    已選
                    {{
                        rooms.filter((room) =>
                            selectedClassroomSet.has(room.id),
                        ).length
                    }}／{{ rooms.length }}
                </span>
            </div>
            ...
            >
                {{
                    rooms.every((room) => selectedClassroomSet.has(room.id))
                        ? '取消全選'
                        : '全選'
                }}
            </button>
        ...
        <!-- Room Grid -->
        <div
            class="grid flex-1 auto-rows-fr grid-cols-2 gap-2 p-4 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5"
        >
            <div v-for="room in rooms" :key="room.id" class="h-full">
                <label
                    class="group/card relative flex h-full cursor-pointer flex-col gap-1 rounded-xl border p-3 transition-all duration-150 focus-within:ring-2 focus-within:ring-primary"
                    :class="
                        selectedClassroomSet.has(room.id)
                ...
                >
                    ...
                    <div class="flex items-center justify-between gap-2">
                        <span
                            class="truncate text-xs font-semibold tracking-wide text-a-text"
                        >
                            {{ room.code }}
                        </span>
                        <div
                        ...
                        >
                            <svg
                                v-if="selectedClassroomSet.has(room.id)"
                    ...
                    <!-- Room name -->
                    <span
                        class="truncate text-[11px] leading-relaxed text-a-text-muted"
                        >{{ room.name }}</span
                    >
